package org.ocean.controller.pay.weixin.bean;
...
public class OrderMessage
{
  ...
  public String sign(String key)
  {
    ...
    p.put("total_fee", String.valueOf(this.total_fee));
    p.put("cash_fee", String.valueOf(this.cash_fee));
    p.put("appid", this.appid);
    p.put("attach", this.attach);
    p.put("bank_type", this.bank_type);
    p.put("cash_fee_type", this.cash_fee_type);
    p.put("coupon_count", this.coupon_count);
    p.put("coupon_fee", this.coupon_fee);
    p.put("coupon_fee_$n", this.coupon_fee_$n);
    p.put("coupon_id_$n", this.coupon_id_$n);
    p.put("device_info", this.device_info);
    p.put("err_code", this.err_code);
    p.put("err_code_des", this.err_code_des);
    p.put("fee_type", this.fee_type);
    p.put("is_subscribe", this.is_subscribe);
    p.put("mch_id", this.mch_id);
    p.put("nonce_str", this.nonce_str);
    p.put("openid", this.openid);
    p.put("out_trade_no", this.out_trade_no);
    p.put("result_code", this.result_code);
    p.put("return_code", this.return_code);
    p.put("return_msg", this.return_msg);
    p.put("time_end", this.time_end);
    p.put("trade_type", this.trade_type);
    p.put("transaction_id", this.transaction_id);
    return Sign.createSign("utf-8", p, key);
  }
